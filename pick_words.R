...
### only keep rating data from full sample ###
#full.data <- full.data[,-c(21:74)]


### just to check that this is approx the same as AMB words... all looks good, length is a little
### long (mean = 5.17 in AMB)
# 
# full.data$VAL <- ifelse(full.data$V.Mean.Sum >= 7, "POS",
#                         ifelse(full.data$V.Mean.Sum <= 3, "NEG", ""))
# 
# full.data$lab <- ifelse(full.data$VAL == "POS", "USE", 
#                         ifelse(full.data$VAL == "NEG", "USE","NO USE"))
# pos <- subset(full.data, VAL == "POS")
# neg <- subset(full.data, VAL == "NEG")
# use <- subset(full.data, lab == "USE")
# 
# sd(use$A.Mean.Sum)
# 4.857481 + 0.9950693
# 4.857481 - 0.9950693
# setwd("~/Desktop")
# write.csv(select, "~/Desktop/WORDS.csv")
# select <- subset(use, A.Mean.Sum <= 5.85255 & A.Mean.Sum >= 3.862412)
# pos <- subset(select, VAL == "POS")
# neg <- subset(select, VAL == "NEG")
...
write.csv(task2, "firstblockpos.csv")
firstblockneg <- mutate_all(firstblockneg, .funs=toupper)
write.csv(task2, "firstblockneg.csv")
